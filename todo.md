# todo

## Part 1

- [x] T1 Scaffold and CLI shell (design 3.1, 11.1).
- [ ] T2 Privacy check, commit-msg hook and CI on three operating systems (design 10.2).
- [ ] T3 Bundle, compiled executables and the smoke test (design 11.1-11.3, 13.3).
- [ ] T4 `chunk` port with fixes K1-K4 (design 3.3, 6.3).
- [ ] T5 `compress` port with fixes K5 and K6 (design 3.4, 6.4).
- [ ] T6 Public design document `docs/design.md`.

## Part 2

- [ ] `depgraph`. Blocked until the design change for review blockers 1-8 is approved.
