...
- **Files with no in-repo importer**: 3
- **Dormant files**: 3
  - **Orphaned (reachable from nothing)**: 3
  - **Test-only (only a test reaches them)**: 0
- **Potentially unused exports**: 4
  - **Unreferenced anywhere**: 4
...
- `src/a.ts`: `unusedConstant`
- `src/dyn.ts`: `dynamicValue`
- `src/orphan.ts`: `forgotten`
- `src/util/index.ts`: `clamp`

### Notes

- `src/a.ts` `unusedConstant`: Checked for a dynamic import() call with a LITERAL string specifier resolving to 'src/a.ts' across the whole repo and found none. This scan can only see import() calls with a literal string specifier -- a dynamic import built from a variable or template literal (e.g. import(`./cmds/${name}.js`)) is invisible to it, so 'found none' means no LITERAL match was found, NOT that nothing imports this. 'unusedConstant' may still be consumed via such a call. Also verify against consumption this scan cannot see at all (docs examples, published API surface, a runtime-built path/`new Worker(...)`) before deleting.
- `src/dyn.ts` `dynamicValue`: Verified: 'src/B.ts' contains a dynamic import() call that resolves to 'src/dyn.ts' -- this stage's static parser cannot see dynamic import(), so 'dynamicValue' may genuinely be consumed there even though nothing else references it. Check 'src/B.ts' before deleting.
- `src/orphan.ts` `forgotten`: Checked for a dynamic import() call with a LITERAL string specifier resolving to 'src/orphan.ts' across the whole repo and found none. This scan can only see import() calls with a literal string specifier -- a dynamic import built from a variable or template literal (e.g. import(`./cmds/${name}.js`)) is invisible to it, so 'found none' means no LITERAL match was found, NOT that nothing imports this. 'forgotten' may still be consumed via such a call. Also verify against consumption this scan cannot see at all (docs examples, published API surface, a runtime-built path/`new Worker(...)`) before deleting.
- `src/util/index.ts` `clamp`: Checked for a dynamic import() call with a LITERAL string specifier resolving to 'src/util/index.ts' across the whole repo and found none. This scan can only see import() calls with a literal string specifier -- a dynamic import built from a variable or template literal (e.g. import(`./cmds/${name}.js`)) is invisible to it, so 'found none' means no LITERAL match was found, NOT that nothing imports this. 'clamp' may still be consumed via such a call. Also verify against consumption this scan cannot see at all (docs examples, published API surface, a runtime-built path/`new Worker(...)`) before deleting.
